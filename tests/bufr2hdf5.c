...
static int write_hdf5_v2_1(odim_polar_t * od, char * file)
{
  hid_t       file_id, dataset_id, dataspace_id, root_id, group_id, attr_id, space_id;  /* identifiers */
  herr_t      status;
  int i, j;
  ...
  status = H5Acreatestring(root_id, "Conventions", Conventions_v2_1);

  if (od->how_string || od->how_double) {
    ...
    fprintf(stderr, "Top how:\n");
    {
      ...
      while (p) {
	fprintf(stderr, "%s %s\n",  p->id, p->value);
	H5Acreatestring(group_id, (char *)(p->id), p->value);
	p = p->next;
      }
    }
    {
      ...
      while (p) {
	fprintf(stderr, "%s %g\n",  p->id, p->value);
	attr_id = H5Acreate(group_id, p->id, H5T_IEEE_F64BE, H5Screate(H5S_SCALAR), H5P_DEFAULT);
	status = H5Awrite(attr_id, H5T_NATIVE_DOUBLE, &p->value);
	status = H5Aclose(attr_id);
	p = p->next;
      }
    }
  ...
  for (i = 0; i < od->nscans; ++i) {
    ...
    if (od->datasets[i].how_string || od->datasets[i].how_double) {
      g_id = H5Gcreate(group_id, "how", 0);
      fprintf(stderr, "===============\n");
      fprintf(stderr, "Dataset%d how:\n", i+1);
      {
	odim_polar_how_string_t * p = od->datasets[i].how_string;
	while (p) {
	  fprintf(stderr, "%s %s\n",  p->id, p->value);
	  H5Acreatestring(g_id, (char *)(p->id), p->value);
	  p = p->next;
	}
      }
      {
	odim_polar_how_double_t * p = od->datasets[i].how_double;
	while (p) {
	  fprintf(stderr, "%s %g\n",  p->id, p->value);
	  attr_id = H5Acreate(g_id, p->id, H5T_IEEE_F64BE, H5Screate(H5S_SCALAR), H5P_DEFAULT);
	  status = H5Awrite(attr_id, H5T_NATIVE_DOUBLE, &p->value);
	  status = H5Aclose(attr_id);
	  p = p->next;
	}
	...
	if (od->datasets[i].dataset_data[j].how_string || od->datasets[i].dataset_data[j].how_double) {
	  g_id = H5Gcreate(data_id, "how", 0);
	  fprintf(stderr, "===============\n");
	  fprintf(stderr, "Data%d how:\n", j+1);
	  {
	    odim_polar_how_string_t * p = od->datasets[i].dataset_data[j].how_string;
	    while (p) {
	      fprintf(stderr, "%s %s\n",  p->id, p->value);
	      H5Acreatestring(g_id, (char *)(p->id), p->value);
	      p = p->next;
	  ...
	  {
	    odim_polar_how_double_t * p = od->datasets[i].dataset_data[j].how_double;
	    while (p) {
	      fprintf(stderr, "%s %g\n",  p->id, p->value);
	      attr_id = H5Acreate(g_id, p->id, H5T_IEEE_F64BE, H5Screate(H5S_SCALAR), H5P_DEFAULT);
	      status = H5Awrite(attr_id, H5T_NATIVE_DOUBLE, &p->value);
...
/******* check command line parameter */
  ...
  
  {
    ...
    if (s1.vltab == 8) {
      ...
      /* fill the HDF5 ODIM data structure */
      if (ok) write_hdf5_v2_1(&odim_data, argv[2]);
    }
